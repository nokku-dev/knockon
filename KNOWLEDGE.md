...
## K-040: Expo の config plugin は自動適用されない — 依存を入れただけでは native 側 (Info.plist / Manifest) の設定が入らず、JS は動くのに実機で落ちる

- **状況**: #253 で App Store 提出前の残作業を洗い出していたところ、`app.json` の `plugins` に `expo-location` が登録されていないことに気付いた（#254 / PR #257）。`expo-location` は `package.json` の依存にあり、`src/location.ts` は正常に import できて型も通り、テストも全て green だった。
- **問題**: config plugin が適用されないと `Info.plist` に `NSLocationWhenInUseUsageDescription` が入らない。iOS は **usage description の無いアプリが位置権限を要求した瞬間にプロセスを強制終了する**。`requestLocationPermission()` は `AnchorEditor` の「現在地を取得」から実際に呼ばれる（`useChainEdit` / `useTodayData` 経由）ので、実機で確実に踏むクラッシュだった。App Store 審査 Guideline 5.1.1 のリスクでもある。
- **原因**: 二重の盲点。(1) **config plugin は autolinking の対象外**。native module の autolinking は自動だが、config plugin (= prebuild で `Info.plist` / `AndroidManifest.xml` を書き換える層) は `app.json` の `plugins` に**明示登録しないと適用されない**。`node_modules/expo-location/expo-module.config.json` にも plugin 宣言は無く、`app.plugin.js` があるだけでは何も起きない。(2) **検証経路に穴があった**。`eas.json` の preview / development の iOS profile は `"simulator": true` で、実機の権限フローを一度も通していなかった（#236 のアイコン検証も表示確認のみ）。JS 層のテストは native 設定を一切見ないので、型チェックもテストも通ったまま欠落が残り続けた。
- **解決**: `app.json` の `plugins` に `["expo-location", { locationWhenInUsePermission, locationAlwaysAndWhenInUsePermission, locationAlwaysPermission }]` を日本語文言つきで追加。`withLocation.js` の `createPermissionsPlugin` は既定値として 3 キーを渡すので、**props 未指定でも 3 つとも必ず書かれる**（既定は英語の `Allow $(PRODUCT_NAME) to access your location`）。前景しか使っていなくても Always 系の英語文言が混入するため 3 つとも日本語で上書きした。`isIosBackgroundLocationEnabled` は立てない（バックグラウンド発火は Phase 1.6b で未実装。立てると `UIBackgroundModes` に `location` が入り、実装していない常時位置利用を審査で説明できなくなる）。回帰は `src/appJsonLocationPlugin.test.ts` で機械固定（[K-006](#k-006) 同型）。
- **教訓**: **`expo-*` の依存を追加したら、その package が config plugin を持つか（`app.plugin.js` の有無）を必ず確認し、持つなら `app.json` の `plugins` に登録する**。「import できる / 型が通る / テストが green」は native 設定が正しいことを一切保証しない — JS 層と native 層は検証経路が分かれている。特に **OS 権限を要求する module（location / camera / microphone / photo library / contacts 等）は usage description の欠落が即クラッシュ**なので危険度が高い。[K-025](#k-025)（Expo モジュール追加は `npx expo install`）は「正しいバージョンを入れる」話だが、本項は「入れた後に config plugin を配線する」話で**別のステップ**。両方やって初めて native 側が揃う。あわせて、simulator ビルドだけで検証を終えると権限系の欠落は永久に見つからないので、**権限を使う機能は実機ビルドで一度は通す**か、本項のように `app.json` をテストで固定して CI に守らせる。

## K-041: Expo 系パッケージの版は npm の `latest` ではなく `bundledNativeModules.json` から取る — dist-tag は常に最新 SDK を指す

- **状況**: #294 で ESLint を導入するため `eslint-config-expo` を入れようとした。npm の最新は `57.0.1`。本プロジェクトは **Expo SDK 54**。
- **問題**: `npm install eslint-config-expo`（= `@latest`）を打つと **SDK 57 向けの設定が入る**。[K-025](#k-025) の native crash と違い、**症状が出ない**。lint は動くしテストも通る。ルールセットと同梱プラグインの版が SDK とずれているだけなので、気付く経路が無い。
- **原因**: 二重の構造。(1) **npm の dist-tag `latest` は「最新の SDK 向け」を指す**。パッケージ側は複数 SDK を並行サポートしないので、古い SDK を使っている側は `@latest` が常に外れる。(2) **`eslint-config-expo` は SDK 55 でバージョン番号を SDK 番号に揃え直した**（`9.x` → `10.x` → **`55.x`** → `56.x` → `57.x`）。番号の連続性から版を推測すると外す — SDK 54 に対応するのは `54.x` ではなく **`~10.0.0`**。`npm view <pkg> versions` を眺めても対応は読めない。
- **解決**: **`node_modules/expo/bundledNativeModules.json` を読む。** これが Expo 自身が持つ SDK ごとのピンで、`"eslint-config-expo": "~10.0.0"` と書いてある（`expo-constants` の `~18.0.13` も同様に確認できる）。`npx expo install <pkg>` は内部でこのマップを参照するので同じ結果になるが、**dev 専用パッケージだと `expo install` を使う発想が出にくい**。マップを直接読めば、入れる前に「何が入るか」を確認できる。
- **教訓**: **Expo 関連パッケージの版を決めるときは、npm ではなく `bundledNativeModules.json` を先に見る。** 対象は `expo-*` の runtime module だけでなく、**`eslint-config-expo` / `jest-expo` / `babel-preset-expo` など SDK に紐づく dev / tooling パッケージも含む**（このリポジトリは `jest-expo` `~54.0.17` / `babel-preset-expo` `^54.0.10` を既に持っている）。[K-025](#k-025) は「`npm install` だと SDK 非互換版が入る」で**症状が実機クラッシュ**、本項は**症状が出ない**のが違い — 壊れないので発見契機が無く、次に SDK を上げるまで気付かない。[K-009](#k-009)（Expo 依存変更時の 4 ファイル diff 確認）と組み合わせて、入れた後に `package.json` / `tsconfig.json` / `babel.config.js` / `app.json` の差分も見る。⚠ 実例は現時点で `eslint-config-expo` の 1 件なので、規約 (CLAUDE.md) や dev-loop-kit テンプレートへの昇格は**実例が増えてから**判断する（実例のない規約は守られない）。

## K-042: Issue に列挙された対象リストは、それ自体が「列挙による検査」— 指示側が列挙で渡してきても走査で検証する

- **状況**: #301 (Phase 1.6b) で OS ジオフェンスを実装した。この作業には #291 の逆操作 (「未実装だから」と後退させた記述を実装に合わせて戻す) が含まれ、Issue 本文に**戻すべき箇所が 9 個、チェックボックスで列挙されていた**。
- **問題**: 9 箇所を全部潰してから `grep` で全走査したところ、**さらに 4 箇所残っていた**。しかも 2 つは**同一ファイル内で、直したばかりの記述と正面から矛盾**していた — `docs/release/app-store-submission.md` は §1.4 を「常時権限を実際に使う」に直したのに、同じファイルの後方に `isIosBackgroundLocationEnabled` は**立てない**」が残っていた。`docs/release/ios-simulator-qa.md` も `A-3 UIBackgroundModes が**不在**` を `[x]` で checked のまま持っていた。残り 2 つは `src/domain.ts` / `src/location.ts` の「Phase 1.6b 後送り」コメント。
- **原因**: **Issue の対象リストは、それ自体が列挙による検査**（[#292](https://github.com/nokku-dev/knockon/issues/292) の `localTimestampGuard` / nokku-ops ADR-0078 と同型の構造）。書き手が記憶から挙げているので、書いた時点の想起漏れがそのまま漏れになる。⚠ 質が悪いのは、**指示として渡されると「確認済みのリスト」に見える**こと — 自分で書いた列挙なら疑うが、他人から渡された列挙は検証をスキップしやすい。「9 箇所」と**数が明示されているとさらに「数は正しい」前提が強化される**。⚠ 加えて**逆操作は構造的に漏れやすい**: 前の作業で触った場所を思い出しながらリストを作るので、当時「ついでに直した」箇所が抜ける。
- **解決**: リストを潰した**後に**、条件の文（「実装に合わせて戻す」）から対象集合を作り直して走査する。今回は `grep -rn "未実装\|Phase 1.6b\|UIBackgroundModes"` を README / SPEC / CLAUDE.md / understanding-map / docs/release / src に当て、ヒットを 1 件ずつ「今も真か」で判定した。**列挙は出発点であって完了条件ではない**。
- **教訓**: **指示側が列挙で渡してきても走査する。** [#292](https://github.com/nokku-dev/knockon/issues/292) は「自分たちが書いたガードの列挙漏れ」、本項は「依頼に書かれた対象の列挙漏れ」で、**同じ構造がコードと依頼の両方で起きる**。全称の条件（「〜と書いてある箇所すべて」）は、全称を走査する判定でしか測れない。実務上の手順は 3 つ: (1) 列挙を潰す、(2) **条件の文から検索語を作り直して走査する**（リストに載っていた語ではなく、条件が指す語）、(3) ヒットを 1 件ずつ真偽判定する。特に「記述を A から B に変える」系の作業は、A の痕跡を検索語にすれば機械的に洗える。⚠ 走査で足りない分が出たら**リストを書いた側に返す**（本項は #301 のリストが 9/13 だったことを PR で明示した）。数が合っていたかどうかは、次に同じ形の依頼を受けるときの重みづけになる。
