...
describe('TodayScreen (PR-X / マルチチェーン + Bottom Sheet)', () => {
  ...
  // Issue #166 (ADR-0042 採択): 採用チェーンが「ちょうど 1 本」のときだけ、
  // 下部に「もう一本テンプレートから足す」ナッジを出す。 K-031 / SPEC §5 step5
  // (「もう一方も?」opt-in) と同型の Augmentation 原則ナッジ (= 摩擦を増やさず
  // 完成形をプレビュー)。 0 本は empty 表示 (新規作成導線) で別経路、 2 本以上は
  // 「もう一本」訴求の役目が終わっているため非表示。
  describe('Issue #166: 採用チェーン 1 本のときの discovery ナッジ', () => {
    test('chains 1 本 + onOpenDiscovery 指定 → 「もう一本」ナッジが表示される', () => {
      const chains: TodayChainData[] = [
        buildChainData('c1', '朝のルーティン', [fireNode('n1', '水を飲む')]),
      ];
      const onOpenDiscovery = jest.fn();
      const { getByLabelText } = render(
        <TodayScreen
          chains={chains}
          onToggleNode={() => {}}
      ...
      expect(getByLabelText('もう一本、テンプレートから足す')).toBeTruthy();
    });

    test('chains 1 本 + ナッジタップ → onOpenDiscovery が呼ばれる', () => {
      const chains: TodayChainData[] = [
        buildChainData('c1', '朝のルーティン', [fireNode('n1', '水を飲む')]),
      ];
      const onOpenDiscovery = jest.fn();
      ...
      fireEvent.press(getByLabelText('もう一本、テンプレートから足す'));
      expect(onOpenDiscovery).toHaveBeenCalledTimes(1);
    });

    test('chains 1 本でも onOpenDiscovery 未指定なら非表示 (発見性 = 親が動線を持つときのみ、 PR-AA 編集ボタンと同型)', () => {
      const chains: TodayChainData[] = [
        buildChainData('c1', '朝のルーティン', [fireNode('n1', '水を飲む')]),
      ];
      ...
      expect(queryByLabelText('もう一本、テンプレートから足す')).toBeNull();
    });

    test('chains 2 本 → 非表示 (「もう一本」訴求の役目が終わっている)', () => {
      const chains: TodayChainData[] = [
        buildChainData('c1', '朝のルーティン', [fireNode('n1', '水')]),
        buildChainData('c2', '夜のルーティン', [fireNode('n2', '歯磨き')]),
      ];
      const { queryByLabelText } = render(
        <TodayScreen
          chains={chains}
    ...
    test('chains 0 本 → 非表示 (empty 表示と排他、 新規作成導線は親 route 側)', () => {
      const { queryByLabelText } = render(
        <TodayScreen chains={[]} onToggleNode={() => {}} onOpenDiscovery={() => {}} />,
      );
      expect(queryByLabelText('もう一本、テンプレートから足す')).toBeNull();
    });
  });
