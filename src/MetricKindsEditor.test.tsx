...
describe('MetricKindsEditor (ADR-0026 PR-CC)', () => {
  ...
  test('open=true で全種別の表示名 + key + 単位を表示', () => {
    ...
    expect(getByText(/key: weight/)).toBeTruthy();
    expect(getByLabelText('気分 を編集')).toBeTruthy();
    expect(getByText(/key: mood/)).toBeTruthy();
  });
  ...
  test('「+ 種別を追加」 → フォーム表示 + 「追加」で onAdd 呼び出し', () => {
    const onAdd = jest.fn();
    ...
    fireEvent.changeText(getByLabelText('key'), 'focus_minutes');
    fireEvent.press(getByLabelText('新規種別保存'));
    expect(onAdd).toHaveBeenCalledWith('focus_minutes', '集中時間', '分');
  });

  test('編集ボタン押下 → 入力欄に既存値、 保存で onUpdate 呼び出し (patch 形式)', () => {
  ...
  test('空文字 (表示名 / 単位 / key の何か空) では追加されない', () => {
    const onAdd = jest.fn();
    const { getByLabelText } = render(
      <MetricKindsEditor
        open={true}
