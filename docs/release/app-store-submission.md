...
> 根拠: 2026-07-21 の判断ログ (iOS 先行・Q1 内・スコープフリーズ)
...
`TODO.md` は「iOS は Android 版 v1 出荷判断の手前までスルー」と書いている (2026-05-19 判断)。一方 2026-07-21 の判断ログは「iOS 先行・Q1 内」への方針転換を含む。**後者 (2026-07-21) が新しく、こちらを優先する**。TODO.md の当該節はリリース完了後に retro で更新する (本 PR では触らない)。
...
| | |
| --- | --- |
| 提出日 | **2026-07-31 17:11 (JST)** |
| バージョン | 1.0.0 (build 1) |
| 提出 ID | `cdeeeafb-4c02-4f1d-b16b-5e042ca0b84d` |
| ステータス | 審査待ち |
| 配信地域 | 日本のみ |
| 価格 | 無料 (ADR-0048) |

**提出時に決めた設定** (次バージョンでも踏襲する方針):

- **Apple シリコン Mac / Apple Vision Pro での配信は無効**。どちらも**一度も動作確認していない**ため。
  「検証していないものを動くと出さない」を貫いた (Info.plist 実測・QA 75 項目と同じ基準)。
  検証すれば後から有効化できる
- **配信は日本のみ**。UI が日本語のみで他国ユーザーは使えず、低評価は**ランキングシグナルとして残る**。
  あわせて EU の trader ステータス申告 (氏名・住所の公開を伴う可能性) と中国の要件も回避できる。
  配信地域は審査不要で後から広げられる
