...
- **テキストオーバーレイ / デバイスフレーム装飾**: v1.0 は素の実機スクショで出す。v1.1 で retro 判断 (#229 §7)。
  - ⚠️ **これは Apple の制約ではなく我々の判断** (#229 §7)。現在の解説では、テキストオーバーレイ・
    デバイスフレーム・背景装飾は許可されているとされる。
  - ⚠️ **ただし「装飾すると通る」と単純化しないこと**。Taku には前職で**加工したスクショが
    リジェクトされた実経験**がある (2026-07-31)。規約文言が変わったのか運用が変わったのかは
    未確認で、審査員による判断のばらつきもある。
  - **両方の時代で共通する安全な線**はこれ: (1) **アプリの実画面が主役**であること、
    (2) **アプリに無い機能を売り込まない**こと。Guideline 2.3.3 が実際に問題にするのはこの 2 点で、
    「実画面がマーケ素材で隠れる」構図が最も危ない。装飾を足すなら実画面の面積を削らない範囲に留める。
  ...
  - **v1 では素のまま出す判断を維持する**。リジェクトは 1 往復で数日失う一方、キーワード一致は
    必要条件でしかなく、順位はダウンロード速度と継続率が支配的。**DL 0 の段階ではどのシグナルを
    積んでも順位はほぼ動かない**ため、実データを見てから投資する方が合理的 (定着の閾値・
    アプリ名の判断と同じ軸)。
