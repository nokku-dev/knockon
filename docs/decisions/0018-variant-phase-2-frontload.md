...
- ノードを variant でスキップした日は、 Today に **「親 title でグレー表示 + スキップマーク (—) + マーカー無し + タップ不可」** の専用行として表示する (PR-1.9 で「設定したのに表示されないとバグと勘違いする」というユーザーフィードバックで決定変更)。 マーカーは描画されないため [ADR-0010](0010-spine-fill-to-last-achieved.md) のスパインは skip ノードの位置を通過するが、 達成済みノード範囲 (= 線が伸びる範囲) の判定対象には入らない (skip は達成済みになり得ない)。 「マイナスを指差さない」原則と整合 (= 「— 」は中立記号、 罰や警告ではない)。 形変化「円→星」の規律にも触れない (skip にはそもそも円が出ない、 表示は文字のみ)。
