...
1. **正準データは 4 軸:** ノード達成 `(ノード, 日付, bool)` + アンカー発火 `(anchor_id, 日付)` + メトリクス `(metric_key, value, recorded_at)` + 手動メモ `(note_id, node_id?, content, created_at, updated_at)` のみ ([ADR-0012](docs/decisions/0012-anchor-firing-events.md) / [ADR-0024](docs/decisions/0024-goal-view-analytics-phase-3-unified.md) / [ADR-0044](docs/decisions/0044-research-manual-notes.md))。**チェーン平均・連鎖の流れ・定着判定・星種別・進捗率・メトリクスの移動平均は一切保存せず表示時に派生計算**する。派生値のカラム追加・キャッシュ・非正規化は禁止。アンカー発火イベント / メトリクス / 手動メモは「観測した事実」として保存 (派生値ではない)。手動メモはユーザーが書いた事実で `node_id` 単一 nullable・ノード削除時は `ON DELETE SET NULL` で本文保全 ([ADR-0044](docs/decisions/0044-research-manual-notes.md))。研究画面はメモ一覧 + FAB、 Today はアクション長押しでメモ作成。アプリ動作設定 (`app_settings` テーブル) は別軸 (= 観測データではなく app config、 [ADR-0028](docs/decisions/0028-app-settings-and-reset-time.md))。
...
4. **禁止 UI（ハードガードレール）:** 格子 / ヒートマップ / カレンダー型ストリーク / ストリーク炎 / 紙吹雪 / 弱い輪・未達のアラート的指差し表示 / ドット塗り率の段階表現は**追加禁止**。左の SVG マーカーは常に円のまま (Issue #118)、 定着の祝福は「連続回数の近くの小さな ★/☆」に置く (旧 v0.3 の「円→星」形変化は撤回)。これらは反 streak / Celebrate 主の核に反する。**例外: 分析タブの二値マトリクス**（達成 = 塗り四角 / 未達 = 極淡 / 休む日 = 空、段階塗り率・赤・連続強調を持たない、#115 / ADR-0037）は禁止の趣旨に触れないため対象外。**判定軸 (#174 / [ADR-0036 §一般原則](docs/decisions/0036-rescind-today-streak-display.md)):** 数字 / グリフ / ハイライトを新規追加するときは「失われうるか (+/-)」で判定する。(+) 失われない指標 (単調増加 / one-way フラグ / 塗り分けしないクロス断面) は採用可、(−) 失われうる指標 (連続日数 / 比率 / 段階塗り率 / 達成状態反映の塗り分け) は追加禁止。グレーは (−) 側に倒す。過去判断対応表は ADR-0036 §一般原則を参照。
