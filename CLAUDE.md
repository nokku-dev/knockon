...
4. **禁止 UI（ハードガードレール）:** 格子 / ヒートマップ / カレンダー型ストリーク / ストリーク炎 / 紙吹雪 / 弱い輪・未達のアラート的指差し表示 / ドット塗り率の段階表現は**追加禁止**。左の SVG マーカーは常に円のまま (Issue #118)、 定着の祝福は「連続回数の近くの小さな ★/☆」に置く (旧 v0.3 の「円→星」形変化は撤回)。これらは反 streak / Celebrate 主の核に反する。**例外: 分析タブの二値マトリクス**（達成 = 塗り四角 / 未達 = 極淡 / 休む日 = 空、段階塗り率・赤・連続強調を持たない、#115 / ADR-0037）は禁止の趣旨に触れないため対象外。
