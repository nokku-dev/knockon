...
4. **禁止 UI（ハードガードレール）:** 格子 / ヒートマップ / カレンダー型ストリーク / ストリーク炎 / 紙吹雪 / 弱い輪・未達のアラート的指差し表示 / ドット塗り率の段階表現は**追加禁止**。**定着ノードの左 SVG マーカーは星型** (ADR-0050、2026-07-07 で Issue #118「常に円」を反転)、未定着ノードは円。星は常に塗り (今日の達成状態で ★/☆ 塗り分けはしない = 反 streak / (−) 禁則と整合)。定着 = 星は Today ドット / 7D ミニマトリクス / 60D マトリクス / 見出し「定着 N 個」に通底させる。**「円→星の形変化は撤回」だった #118 自体を ADR-0050 で再反転した** (実機判断: 定着はドット自体を星にする)。段階塗り率 / ストリーク炎等の禁止は不変。**例外: 分析タブの二値マトリクス**（達成 = 塗り四角 / 未達 = 極淡 / 休む日 = 空、段階塗り率・赤・連続強調を持たない、#115 / ADR-0037）は禁止の趣旨に触れないため対象外。**判定軸 (#174 / [ADR-0036 §一般原則](docs/decisions/0036-rescind-today-streak-display.md)):** 数字 / グリフ / ハイライトを新規追加するときは「失われうるか (+/-)」で判定する。(+) 失われない指標 (単調増加 / one-way フラグ / 塗り分けしないクロス断面) は採用可、(−) 失われうる指標 (連続日数 / 比率 / 段階塗り率 / 達成状態反映の塗り分け) は追加禁止。グレーは (−) 側に倒す。過去判断対応表は ADR-0036 §一般原則を参照。
