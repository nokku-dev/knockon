...
1. **正準データは 3 軸:** ノード達成 `(ノード, 日付, bool)` + アンカー発火 `(anchor_id, 日付)` + メトリクス `(metric_key, value, recorded_at)` のみ ([ADR-0012](docs/decisions/0012-anchor-firing-events.md) / [ADR-0024](docs/decisions/0024-goal-view-analytics-phase-3-unified.md))。**チェーン平均・連鎖の流れ・定着判定・星種別・進捗率・メトリクスの移動平均は一切保存せず表示時に派生計算**する。派生値のカラム追加・キャッシュ・非正規化は禁止。アンカー発火イベント / メトリクスは「観測した事実」として保存 (派生値ではない)。アプリ動作設定 (`app_settings` テーブル) は別軸 (= 観測データではなく app config、 [ADR-0028](docs/decisions/0028-app-settings-and-reset-time.md))。
