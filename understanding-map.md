...
- **正準データ5軸**: ノード達成 `(node, date, bool)` / アンカー発火 `(anchor_id, date)` / メトリクス `(metric_key, value, recorded_at)` / 手動メモ `(note_id, node_id?, content, …)` / 定着取り下げ `(node_id, retracted_at)`。派生値（連鎖の流れ・定着判定・進捗率・星種別）は一切保存せず表示時に派生計算（ADR-0001 / 0012 / 0024 / 0044 / 0047）。定着は生涯マイルストーン(latch)だが派生のまま。定着ノードは Today で ★ 表示・**auto-✓・タップ不要**（effective=実レコードOR定着、タップ no-op＝レコード非書込・K-002。判断二転: 当初✓自動→07-06タップ従来どおり→07-07 auto-✓に再反転＝手間削減 Augmentation）。累計・60Dマトリクスは派生で数える。取り下げ導線＝定着ノード長押しメニュー + ログ定着ポートフォリオ（#205）。ステージは4段（これから/育成中/もう少しで定着/定着）+週次流入。
