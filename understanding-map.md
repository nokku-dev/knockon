...
- **正準データ5軸**: ノード達成 `(node, date, bool)` / アンカー発火 `(anchor_id, date)` / メトリクス `(metric_key, value, recorded_at)` / 手動メモ `(note_id, node_id?, content, …)` / 定着取り下げ `(node_id, retracted_at)`。派生値（連鎖の流れ・定着判定・進捗率・星種別）は一切保存せず表示時に派生計算（ADR-0001 / 0012 / 0024 / 0044 / 0047）。定着は生涯マイルストーン(latch)だが派生のまま。定着ノードのTODAY自動達成は達成レコードを書かない（表示のみ・K-002）。
