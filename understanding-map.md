...
- **正準データ5軸**: ノード達成 `(node, date, bool)` / アンカー発火 `(anchor_id, date)` / メトリクス `(metric_key, value, recorded_at)` / 手動メモ `(note_id, node_id?, content, …)` / 定着取り下げ `(node_id, retracted_at)`。派生値（連鎖の流れ・定着判定・進捗率・星種別）は一切保存せず表示時に派生計算（ADR-0001 / 0012 / 0024 / 0044 / 0047）。定着は生涯マイルストーン(latch)だが派生のまま。定着ノードは Today で ★ 表示・タップ従来どおり（auto 達成なし＝レコード非書込・K-002。ADR-0047 当初の「✓自動・タップ不要」は 2026-07-06 撤回）。取り下げ導線＝定着ノード長押しメニュー + ログ定着ポートフォリオ（#205）。
