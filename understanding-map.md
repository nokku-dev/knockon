...
> 最終更新: 2026-07-29（SPEC v0.4 / ADR-0051 まで反映。リリーススコープ = ADR-0045 / 0047 / 0048 / 0049 時点）。
...
- **発火ロジック**: Today に並ぶ＝ active な全チェーンのノード。アンカーは通知＋「発火中」ピルのための分類軸（ADR-0020 で旧「手動発火」概念は廃止）。場所発火は OS 標準ジオフェンス（有料 API 不要）。#301 で region monitoring を実装済み（`geofencing.ts`）。バックグラウンド到達と、Today を開いたときの前景判定の二重経路で、どちらも ADR-0012 の 1 日 1 回に収束する。
...
- **expo-location / expo-task-manager** — 前景の現在地取得（`location.ts`）と region monitoring（`geofencing.ts` / 登録対象の決定は純関数 `geofenceRegions.ts`）。
